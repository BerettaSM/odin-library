var BookService = (function () {
    ...
    function createBook(json) {
        ...
        return new Book(
            ...
            first_sentence
                ? first_sentence[0]
                : 'A description/first sentence was not provided.',
            json.ratings_average != undefined
                ? json.ratings_average.toFixed(1)
                : 'Unknown rating',
    ...
    async function searchBooks(query = '', page = 1, limit = 20) {
        var fullURL = `${searchURL}?q=${encodeURIComponent(
            query
        )}&page=${page}&limit=${limit}&fields=${encodeURIComponent(fields)}`;
